...
class Harness:
    def __init__(self, initial_state, config_dir=None, CscClass=salobj.TestCsc):
        ...
        self.csc = CscClass(
            index=index, config_dir=config_dir, initial_state=initial_state
        )
...
class CommunicateTestCase(asynctest.TestCase):
    ...
    async def test_setArrays_command(self):
        async with Harness(initial_state=salobj.State.ENABLED) as harness:
            ...
            logLevel = await harness.remote.evt_logLevel.next(
            ...
            log_message = await harness.remote.evt_logMessage.next(
                flush=False, timeout=STD_TIMEOUT
            )
            ...
            evt_data = await harness.remote.evt_arrays.next(
                flush=False, timeout=STD_TIMEOUT
            )
            harness.csc.assert_arrays_equal(cmd_data_sent, evt_data)
            tel_data = await harness.remote.tel_arrays.next(
                flush=False, timeout=STD_TIMEOUT
            )
            ...
            # also test get
            harness.csc.assert_arrays_equal(
                cmd_data_sent, harness.remote.tel_arrays.get()
            )
            harness.csc.assert_arrays_equal(
                cmd_data_sent, harness.remote.evt_arrays.get()
            )
    ...
    async def test_setScalars_command(self):
        async with Harness(initial_state=salobj.State.ENABLED) as harness:
            ...
            await harness.remote.cmd_setScalars.start(
                cmd_data_sent, timeout=STD_TIMEOUT
            )
            log_message = await harness.remote.evt_logMessage.next(
                flush=False, timeout=STD_TIMEOUT
            )
            ...
            evt_data = await harness.remote.evt_scalars.next(
                flush=False, timeout=STD_TIMEOUT
            )
            harness.csc.assert_scalars_equal(cmd_data_sent, evt_data)
            tel_data = await harness.remote.tel_scalars.next(
                flush=False, timeout=STD_TIMEOUT
            )
            ...
            # also test get
            harness.csc.assert_scalars_equal(
                cmd_data_sent, harness.remote.tel_scalars.get()
            )
            harness.csc.assert_scalars_equal(
                cmd_data_sent, harness.remote.evt_scalars.get()
            )
        ...
        * exitControl: STANDBY or FAULT to OFFLINE (quit)
        ...
        async with Harness(initial_state=salobj.State.STANDBY) as harness:
            for state in salobj.State:
                ...
                data = await harness.remote.evt_errorCode.next(
                    flush=False, timeout=STD_TIMEOUT
                )
    ...
    async def test_fault_method(self):
        ...
        async with Harness(initial_state=salobj.State.STANDBY) as harness:
            ...
            with self.assertRaises(asyncio.TimeoutError):
                await harness.remote.evt_errorCode.next(
        ...
        for doraise, report_first in itertools.product((False, True), (False, True)):
            with self.subTest(doraise=doraise, report_first=report_first):
                async with FailInReportFaultCsc(
                    ...
                    state = await remote.evt_summaryState.next(
                        flush=False, timeout=STD_TIMEOUT
                    )
                    ...
                    self.assertEqual(state.summaryState, salobj.State.FAULT)
                    data = await remote.evt_errorCode.next(
                        flush=False, timeout=STD_TIMEOUT
                    )
                    self.assertEqual(data.errorCode, code)
                    self.assertEqual(data.errorReport, report)
    ...
    async def test_simulation_mode(self):
        ...
        async with Harness(initial_state=salobj.State.STANDBY) as harness:
            sm_data = await harness.remote.evt_simulationMode.next(
                flush=False, timeout=LONG_TIMEOUT
            )
            ...
            for state in (
                ...
                with self.subTest(state=state):
                    await salobj.set_summary_state(
                        remote=harness.remote, state=salobj.State.DISABLED
                    )
    ...
    async def test_initial_simulation_mode(self):
        ...
        async with salobj.TestCsc(
            index=1, config_dir=TEST_CONFIG_DIR, initial_simulation_mode=0
        ) as csc:
    ...
    async def check_simulate_mode_ok(self, harness):
        ...
        """
        await harness.remote.cmd_setSimulationMode.set_start(
            mode=0, timeout=STD_TIMEOUT
        )
        sm_data = await harness.remote.evt_simulationMode.next(
            flush=False, timeout=STD_TIMEOUT
        )
        ...
        for bad_mode in (1, 10, -1):
            with self.subTest(bad_mode=bad_mode):
                with salobj.assertRaisesAckError():
                    await harness.remote.cmd_setSimulationMode.set_start(
                        mode=bad_mode, timeout=STD_TIMEOUT
                    )
...
class ConfigurationTestCase(asynctest.TestCase):
    ...
    async def test_no_config_specified(self):
        async with Harness(
            ...
            data = await harness.remote.evt_settingVersions.next(
                flush=False, timeout=STD_TIMEOUT
            )
            self.assertEqual(
                pathlib.Path(TEST_CONFIG_DIR).resolve().as_uri(), data.settingsUrl
            )
...
class ControllerCommandLoggingTestCase(asynctest.TestCase):
    ...
    async def test_logging(self):
        async with Harness(
            ...
            msg = await harness.remote.evt_logMessage.next(
                flush=False, timeout=STD_TIMEOUT
            )
            ...
            with self.assertRaises(asyncio.TimeoutError):
                await harness.remote.evt_logMessage.next(
                    flush=False, timeout=NODATA_TIMEOUT
                )

            await harness.remote.cmd_setLogLevel.set_start(
