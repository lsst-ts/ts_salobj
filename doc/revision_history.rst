...
* `AsyncS3Bucket` now reads environment variable ``S3_ENDPOINT_URL`` to obtain the endpoint URL.
  This change is required in order to use the NCSA S3 instance.
* Added an ``Environment Variables`` section to the main documentation page ``index.rst``.

Minor changes:

* Added a unit test for `QueueCapacityChecker`.
