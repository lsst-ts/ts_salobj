...
* Replace `duration` with `timespan` in Test and Script events, because `duration` is a reserved keyword in InfluxDB.

Other changes:

* Fix a call to `warnings.warn` Domain which was misspelled as `warnings.warning`.
