...
def _get_dds_descriptor_from_idl(idl_path, type_name):
    ...
    if topictype is None:
        raise RuntimeError(
            "Specified IDL file does not define topic type: " + type_name
        )
        ...
        if key not in _class_dict:
            ...
            for child in element:
                if child.tag == _ELEMENT_TAG:
                    enum_literals[child.attrib[_NAME_ATTRIBUTE]] = int(
                        child.attrib[_VALUE_ATTRIBUTE]
                    )
...
def _deserialize_sequence(mem_type, data):
    ...
    if seq_size != 0:
        ...
        [
            result.append(_deserialize_data(actual_type, unpacked))
            for _ in range(seq_size)
        ]
    return result
...
def _dds_type_support(descriptor, type_name, keys, data_class):
    ...
    def __init__(self):
        ...
        dds.TypeSupport.__init__(
            self, type_name, keys, descriptor, struct.calcsize(self._packing_format)
        )
...
def _deserialize_seq(data, seq_content_fmt, helper):
    ...
    next(data)  # seq 'free' flag - dont' care about this value
    if buffer_pointer != 0:
        buffer = _ptr_to_bytes(
            buffer_pointer, struct.calcsize(seq_content_fmt) * seq_length
        )
...
_longlong_checker = _int_checker(
    _IntSizes.LONGLONG_MIN.value, _IntSizes.LONGLONG_MAX.value
)
...
def _bounded_str_checker(bound):
    def checker(self, value):
        ...
        if bound > 0 and len(value) > bound:
            raise TypeError(
                "String length of {} exceeds maximum allowed of {}".format(
                    len(value), bound
                )
            )
        try:
