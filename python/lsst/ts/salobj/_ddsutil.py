...
def _compute_cls_packing_fmt(o, fmt_args):
    ...
    for mem_type in o._members.values():
        align = max(align, _compute_packing_fmt(mem_type, sub_fmt_args, align))

    fmt_we_are_being_embedded_into = ''.join(fmt_args)
    sub_fmt = ''.join(sub_fmt_args)
    pre_struct_padding = _align(fmt_we_are_being_embedded_into, sub_fmt)

    fmt_args.extend(pre_struct_padding)
    sz = struct.calcsize(sub_fmt)
    paddings = _padding(sz, align)
    if paddings :
...
def _deserialize_seq(data, seq_content_fmt, helper):
    ...
    next(data) # seq 'free' flag - dont' care about this value
    if seq_length != 0 and buffer_pointer != 0:
...
def _base_checker(cast):
    def checker(self, value):
        # if cast cannot be performed will raise a TypeError
        try:
            return cast(value)
        except ValueError as e:
            raise TypeError('invalid value') from e
    return checker

_float_checker = _base_checker(float)
_int_checker = _base_checker(int)
_bool_checker = _base_checker(bool)

def _range_checker(minval, maxval):
    def checker(self, value):
        try:
            value = int(value)
        except ValueError as e:
...
_octet_checker = _range_checker(0, _IntSizes.OCTET_MAX.value)
_ushort_checker = _range_checker(0,_IntSizes.USHORT_MAX.value)
_ulong_checker = _range_checker(0,_IntSizes.ULONG_MAX.value)
_ulonglong_checker = _range_checker(0,_IntSizes.ULONGLONG_MAX.value)
_short_checker = _range_checker(_IntSizes.SHORT_MIN.value,_IntSizes.SHORT_MAX.value)
_long_checker = _range_checker(_IntSizes.LONG_MIN.value,_IntSizes.LONG_MAX.value)
_longlong_checker = _range_checker(_IntSizes.LONGLONG_MIN.value,_IntSizes.LONGLONG_MAX.value)


def _unbounded_seq_checker(inner_checker):
    def checker(self, value):
        return [inner_checker(self, v) for v in value]
    return checker


def _seq_checker(bound, inner_checker):
    s = _unbounded_seq_checker(inner_checker)

    def checker(self, value):
        if bound > 0 and len(value) > bound:
            raise TypeError('must have maximum length of {}'.format(bound))
        return s(self, value)
...
def _array_checker(bound, inner_checker):
    ...
    def checker(self, value):
        if len(value) != bound:
            raise TypeError("must have a length of exactly {}.".format(bound))
        return s(self, value)
...
def _char_checker(self, value):
    try:
        v = value.encode(DDS_STRING_ENCODING)
        assert type(v) == bytes and len(v) == 1
    except AssertionError as e:
        raise TypeError('must contain exactly one character') from e
    except AttributeError as e:
        raise TypeError('must support encoding') from e
    except UnicodeEncodeError as e:
        raise TypeError('must encode to {}'.format(DDS_STRING_ENCODING)) from e
    return value


def _str_checker(self, value):
    try:
        v = value.encode(DDS_STRING_ENCODING)
        assert type(v) == bytes
    except AssertionError as e:
        raise TypeError('must be a string that encodes to bytes') from e
    except AttributeError as e:
        raise TypeError('must support encoding') from e
    except UnicodeEncodeError as e:
        raise TypeError('must encode to {}'.format(DDS_STRING_ENCODING)) from e
...
def _bounded_str_checker(bound):
    def checker(self, value):
        try:
            v = value.encode(DDS_STRING_ENCODING)
            assert type(v) == bytes and len(value) <= bound
        except AssertionError as e:
            raise TypeError('must be a string that encodes to bytes with less than {} characters'.format(bound)) from e
        except AttributeError as e:
            raise TypeError('must support encoding') from e
        except UnicodeEncodeError as e:
            raise TypeError('must encode to {}'.format(DDS_STRING_ENCODING)) from e
